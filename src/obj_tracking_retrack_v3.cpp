...
bool matching_fn(Mat frame, Mat templ, int match_method, Mat &result = *(new Mat), Point &matchloc = *(new Point))
    ...
    while (cap.read(frame))
    {
        ...
        if (ok)
        {
            ...
            try
            {
                ...
                bool matching_OK = matching_fn(croppedRef, backuped_frames[15], match_method, result);
                double minVal, maxVal;
                minMaxLoc(result, &minVal, &maxVal);
                double match_quality = 0.85;
                if ((match_method == CV_TM_SQDIFF_NORMED && minVal < (1 - match_quality)) || (match_method != CV_TM_SQDIFF_NORMED && maxVal > match_quality))
                {
