...
int main(int argc, char **argv)
{
    ...
    while (cap.read(frame))
    {
        ...
        if (ok)
        {
            ...
            {
                imshow("Target Exctracted", backuped_frames[((int)cap.get(CV_CAP_PROP_POS_FRAMES) - 1) % 15]);
            }
...
#ifdef ENABLE_RETRACKING
            {
                ...
                for (int i = -1; i < 15; i++)
                {
                    ...
                    if (i == -1)
                        ...
                        templ = backuped_frames[((int)cap.get(CV_CAP_PROP_POS_FRAMES) - 7 + i) % 15];
                    ...
                    if ((match_method == CV_TM_SQDIFF_NORMED && minVal < (1 - match_quality)) || (match_method != CV_TM_SQDIFF_NORMED && maxVal > match_quality))
                    {
                        ...
                        bbox = Rect2d(matchLoc, Point(matchLoc.x + templ.cols, matchLoc.y + backuped_frames[((int)cap.get(CV_CAP_PROP_POS_FRAMES) - 7 + i) % 15].rows));
